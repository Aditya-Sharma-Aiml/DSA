...
using namespace std;
  ...
  public:
      ...
      vector<int> findPeakGrid(vector<vector<int>>& arr) {
          ...
          while (low <= high) {
              int mid = (low + high) / 2; // column_index
  
              // Find the index of the row with the maximum element 
              // in the middle column
              int row = maxElement(arr, mid);//index of rowArray
              ...
              // Check if the middle element is greater than its neighbors => apply 1D peak element concept
